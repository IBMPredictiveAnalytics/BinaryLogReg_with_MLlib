script_details = ("logistic_regression_score.py",0.5)
...
if ascontext:
    if ascontext.isComputeDataModelOnly():
        ...
        model_details = json.loads(ascontext.getModelContentToString("model"))
else:
    ...
    model_details = json.loads(open(modelpath,"r").read())
...
if sc.version.find("1.3") != 0:
    model = LogisticRegressionModel(model_details["weights"],model_details["intercept"],len(model_details["weights"]),2)
else:
    model = LogisticRegressionModel(model_details["weights"],model_details["intercept"])
...
rdd1 = dmt.extractDenseVector(df,model_metadata["predictors"],1.0)
...
def rowToList(row):
        ...
        return result
rdd2 = rdd1.map(lambda x:rowToList(x[0])+[datamodel[target][int(model.predict(x[1]))]])
